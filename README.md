[Leia em Português](#portugues) | [Read in English](#english)
...
Ele compila o React no modo de produção e otimiza a compilação para obter o melhor desempenho.
...
**Observação: esta é uma operação unilateral. Uma vez que você a execute, não poderá voltar atrás!**

Se você não estiver satisfeito com a ferramenta de compilação e as opções de configuração, você pode "ejetar" a qualquer momento. Este comando removerá a única dependência de compilação do seu projeto.
...
Você não precisa usar `eject`. O conjunto de recursos cuidadosamente elaborado é adequado para implantações pequenas e médias, e você não deve se sentir obrigado a usar este recurso. No entanto, entendemos que essa ferramenta não seria útil se você não pudesse personalizá-la quando estiver pronto.
